...
Purpose:
- verify the frozen canonical input and the accepted 19-step transform manifest;
...
def load_manifest(repo_root: Path) -> dict:
    ...
    require(len(steps) == 19, f"manifest must contain exactly 19 steps, found {len(steps)}")
    nums = [s.get("step") for s in steps]
    require(nums == list(range(1, 20)), f"manifest step order invalid: {nums}")
    require(len({s.get("path") for s in steps}) == 19, "manifest contains duplicate executable paths")
...
def reconstruct(repo_root: Path, work_dir: Path, output: Path) -> None:
    ...
    require(len(executed) == 19, f"expected 19 executed transforms, found {len(executed)}")
...
def main() -> None:
    ...
    if args.command == "preflight":
        ...
        print("ACCEPTED_TRANSFORMS_AVAILABLE=19")
