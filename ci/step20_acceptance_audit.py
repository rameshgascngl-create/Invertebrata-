...
async def overlay(page,fid):
 return await page.evaluate('''fid=>{const f=document.querySelector('figure[data-v188-plate="'+CSS.escape(fid)+'"]'),src=f?.querySelector('svg,img'),o=document.querySelector('#invSyllabusVisualOverlay'),m=o?.querySelectorAll('.theory-zoom-media')||[];let same=false;if(src&&m[0]&&src.tagName===m[0].tagName)same=src.tagName.toLowerCase()==='svg'?src.innerHTML===m[0].innerHTML&&src.getAttribute('viewBox')===m[0].getAttribute('viewBox'):src.getAttribute('src')===m[0].getAttribute('src');return {visible:!!o&&!o.hidden,count:m.length,identity:same}}''',fid)
...
async def main_async(args):
 ...
 step19=Path(args.step19).read_text(encoding='utf-8');step20a=Path(args.step20a).read_text(encoding='utf-8');step20b=Path(args.step20b).read_text(encoding='utf-8')
 if step20a!=step20b: raise SystemExit('Run A/B HTML differ')
 async with async_playwright() as p:
  browser=await p.chromium.launch(headless=True,executable_path=args.chromium,args=['--no-sandbox','--disable-dev-shm-usage'])
  bi,br,be,_=await collect(browser,step19);ci,cr,ce,applied=await collect(browser,step20a)
  sa=await phase_b2(browser,step20a,ci,'A',out);sb=await phase_b2(browser,step20b,ci,'B',out)
  await browser.close()
 ...
 inv={'step19_occurrences':len(bi),'step20_occurrences':len(ci),'step19_unique':len(binv),'step20_unique':len(cinv),'missing':sorted(set(binv)-set(cinv)),'extra':sorted(set(cinv)-set(binv)),'duplicates':{k:v for k,v in Counter(x['id'] for x in ci).items() if v>1},'markup_changes':[fid for fid in binv if fid in cinv and binv[fid]['html']!=cinv[fid]['html']],'caption_changes':[fid for fid in binv if fid in cinv and binv[fid]['caption']!=cinv[fid]['caption']]}
 ...
 with (out/'STEP20_PRECONDITION_DESTINATIONS.csv').open('w',newline='',encoding='utf-8') as f:w=csv.DictWriter(f,fieldnames=list(pre[0]));w.writeheader();w.writerows(pre)
 with (out/'STEP20_POSTCONDITIONS.csv').open('w',newline='',encoding='utf-8') as f:w=csv.DictWriter(f,fieldnames=list(post[0]));w.writeheader();w.writerows(post)
 ...
 with (out/'FULL_CONTEXTUAL_AUDIT_STEP20.csv').open('w',newline='',encoding='utf-8') as f:w=csv.DictWriter(f,fieldnames=list(full[0]));w.writeheader();w.writerows(full)
 summary={'preconditions_pass':all(x['pass'] for x in pre),'postconditions_pass':all(x['pass'] for x in post),'inventory':inv,'ownership_migrations':ownership,'non_target_drift_count':sum(not x['pass'] for x in non),'placement_distribution':layout,'step20_runtime_applied_ids':sorted(applied['ids']),'step20_runtime_fatal':applied['fatal'],'phase_b2_A':sa,'phase_b2_B':sb,'blocker_c_remediation_pass':sum(x['pass'] for x in post),'full_contextual_audit_pass':all(x['pass'] for x in post) and sum(not x['pass'] for x in non)==0 and layout['hidden']==0 and layout['closed_details']==0 and layout['overflow_failures']==0 and layout['runtime_duplicates']==0 and not layout['runtime_errors'] and sa['pass'] and sb['pass']}
 summary['pass']=all([summary['preconditions_pass'],summary['postconditions_pass'],inv['step19_occurrences']==82,inv['step20_occurrences']==82,inv['step19_unique']==82,inv['step20_unique']==82,not inv['missing'],not inv['extra'],not inv['duplicates'],not inv['markup_changes'],not inv['caption_changes'],ownership==[{'figure_id':'peripatus','from':'u4-modes-life','to':'u4-peripatus'}],summary['non_target_drift_count']==0,layout['contextual_visible_matches']==68,layout['neutral_original_container_fallbacks']==14,layout['neutral_lesson_anchor_fallbacks']==0,layout['hidden']==0,layout['closed_details']==0,layout['overflow_failures']==0,layout['runtime_duplicates']==0,not layout['runtime_errors'],sorted(applied['ids'])==sorted(TARGETS),applied['fatal'] is None,sa['pass'],sb['pass'],summary['full_contextual_audit_pass']])
 (out/'STEP20_ACCEPTANCE_AUDIT.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
 report=['# INVERTEBRATA Step-20 Acceptance Audit','',f"Overall: {'PASS' if summary['pass'] else 'FAIL'}",'',f"Precondition destinations: {'PASS' if summary['preconditions_pass'] else 'FAIL'}",f"Postconditions: {'PASS' if summary['postconditions_pass'] else 'FAIL'}",f"Blocker-C remediation: {summary['blocker_c_remediation_pass']}/9 PASS",f"Non-target drift: {summary['non_target_drift_count']}",f"Placement distribution: {dict(layout)}",f"Phase-B2 Run A: {'PASS' if sa['pass'] else 'FAIL'}",f"Phase-B2 Run B: {'PASS' if sb['pass'] else 'FAIL'}",f"Full contextual audit: {'PASS' if summary['full_contextual_audit_pass'] else 'FAIL'}"]
 (out/'STEP20_ACCEPTANCE_AUDIT_REPORT.md').write_text('\n'.join(report)+'\n',encoding='utf-8')
 raise SystemExit(0 if summary['pass'] else 2)

def main():
 ap=argparse.ArgumentParser();ap.add_argument('--step19',required=True);ap.add_argument('--step20a',required=True);ap.add_argument('--step20b',required=True);ap.add_argument('--out',required=True);ap.add_argument('--chromium',required=True);args=ap.parse_args();raise SystemExit(asyncio.run(main_async(args)))
